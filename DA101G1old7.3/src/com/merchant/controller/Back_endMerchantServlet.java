...
@WebServlet("/Back_endMerchantServlet1")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class Back_endMerchantServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("back-end/merchant/merchant_select_page.jsp");
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllMerchant.jsp的請求
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("merchantVO", merchantVO);         // 資料庫取出的merchantVO物件,存入req
				String url = "back-end/merchant/update_merchant_input.jsp";
		...
		if ("update".equals(action)) { // 來自update_merchant_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("back-end/merchant/update_merchant_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
        ...
        if ("insert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("merchantVO", merchantVO); // 含有輸入格式錯誤的merchantVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("back-end/merchant/addMerchant.jsp");
					failureView.forward(req, res);
					return;
				}
