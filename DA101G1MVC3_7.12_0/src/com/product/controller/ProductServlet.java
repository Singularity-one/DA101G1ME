			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("back-end/product/backEndProduct_select_page.jsp");
...
//		}
      ...
      if ("get_OneStatus_Product".equals(action)) { // 來自backEndProductselect_page.jsp的請求
      	...
      	try {
      		...
      		/***************************3.查詢完成,準備轉交(Send the Success view)*************/
      		req.setAttribute("List<ProductVO>", list); // 資料庫取出的empVO物件,存入req
      		String url = "back-end/product/listOneStatusOfProduct.jsp";
      		RequestDispatcher successView = req.getRequestDispatcher(url); // 成功轉交 listOneEmp.jsp
      		successView.forward(req, res);
